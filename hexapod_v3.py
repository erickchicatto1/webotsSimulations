...
class Hexapod(Robot):
    ...
    def generate_walking_sequence_ik(self, paras: dict):
        ...
        legs = set(n.split('_')[0] for n in self.motors.keys())

        for leg in legs:
            coxa_stance = self._extract_segment(stance_pose, leg, 'COXA')
            coxa_swing = self._extract_segment(swing_pose, leg, 'COXA')
            femur_stance = self._extract_segment(stance_pose, leg, 'FEMUR')
            femur_swing = self._extract_segment(swing_pose, leg, 'FEMUR')
            tibia_stance = self._extract_segment(stance_pose, leg, 'TIBIA')
            tibia_swing = self._extract_segment(swing_pose, leg, 'TIBIA')

            if leg in ('MR', 'FL', 'HL'):
                coxa_seq = coxa_stance + coxa_swing
                femur_seq = femur_stance + femur_swing
                tibia_seq = tibia_stance + tibia_swing
            else:
                coxa_seq = coxa_swing + coxa_stance
                femur_seq = femur_swing + femur_stance
                tibia_seq = tibia_swing + tibia_stance
    ...
    def _extract_segment(self, pose_list, leg, seg):
        values = []
        for p in pose_list:
            values.append(p[leg][seg])
        return values
